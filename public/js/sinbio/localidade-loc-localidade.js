...
function setProjetoPrograma(){
    $.post('/sitio-projetos-programas/verifica-permissao', {
        sOP: "Cadastrar"
    }, function(data) {
        if (data) {
...
function novoUsuario() {
    $.post('/localidade/verifica-permissao', {
        sOP: "Cadastrar"
    } ,function(data) {
        if (data) {
            $(window.document.location).attr('href',"/localidade/cadastrar");
        }
        else {
            jAlert("Voce não possui permissão de acessar essa área.","Acesso Negado");
        }
    ...
    $.post('/localidade/verifica-permissao', {
        ...
        if (data) {
            if ($('.trSelected').length > 1) {
                ...
                var nId = $('.trSelected td:first-child div').text();
                $(window.document.location).attr('href',"/localidade/alterar/nId/"+nId);
            }
        }
        else {
            jAlert("Voce não possui permissão de acessar essa área.","Acesso Negado");
...
function excluirUsuario() {
    if ($('.trSelected').length == 0) {
        ...
        if (confirm('Você tem certeza que deseja excluir ' + $('.trSelected').length + ' Localidade(s)?')) {
            $.post('/localidade/verifica-permissao', {
                sOP: "Excluir"
            } ,function(data) {
                if (data) {
